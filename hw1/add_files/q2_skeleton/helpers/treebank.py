...
class StanfordSentiment:
    def __init__(self, path=None, tablesize = 1000000):
        if not path:
            path = join(getcwd(),"helpers","datasets","stanfordSentimentTreebank")
    ...
    def sent_labels(self):
        ...
        dictionary = dict()
        phrases = 0
        with open(join(self.path,"dictionary.txt"), "r") as f:
            for line in f:
                line = line.strip()
        ...
        labels = [0.0] * phrases
        with open(join(self.path,"sentiment_labels.txt"), "r") as f:
            first = True
            for line in f:
                if first:
                    first = False
    ...
    def dataset_split(self):
        ...
        with open(join(self.path,"datasetSplit.txt"), "r") as f:
            first = True
            for line in f:
                if first:
                    first = False
